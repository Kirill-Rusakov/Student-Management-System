...
@section('content')
    <form action="{{route('students.store')}}" method="post" class="mt-5">
        @csrf
        <h2 class="mb-3 text-dark">Add student</h2>
        <div class="mb-3">
            <label for="name" class="form-label text-dark">Name</label>
            <input type="text" name="name" class="form-control border-light bg-dark text-light" id="name">
            @error('name')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="surname" class="form-control border-light bg-dark text-light" id="surname">
            @error('surname')
                <div class="text-danger bg-light">{{$message}}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="number" name="age" class="form-control border-light bg-dark text-light" id="age">
            @error('age')
                <div class="text-danger bg-light">{{$message}}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <select class="form-select border-light bg-dark text-light" name="group_id" id="group">
                @foreach($groups as $group)
                    <option class="text-light" value="{{$group->id}}">{{$group->title}}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-5">
            ...
            <select class="form-select border-light bg-dark text-light" multiple name="lessons[]" id="lessons">
                @foreach($lessons as $lesson)
                    <option class="text-light" value="{{$lesson->id}}">{{$lesson->title}}</option>
                @endforeach
            </select>
        </div>
